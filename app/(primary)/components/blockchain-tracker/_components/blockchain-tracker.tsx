  ...
  return (
    <div className={`relative h-[20rem] w-full max-w-[500px] rounded-md border overflow-hidden ${
      ...
      {blocks.map((block, i) => (
        ...
        >
          <motion.div
            ...
            animate={{
              borderColor: activeBlocks.includes(i) ? "#06b6d4" : (isDark ? "#525252" : "#9ca3af"),
              backgroundColor: activeBlocks.includes(i) ? (isDark ? "#0f172a" : "#f0f9ff") : (isDark ? "#171717" : "#f9fafb"),
              color: activeBlocks.includes(i) ? "#06b6d4" : (isDark ? "#a3a3a3" : "#6b7280"),
      ...
      <div className="absolute top-4 left-4 flex items-center gap-2">
        ...
        />
        <span className={`text-xs ${isDark ? 'text-neutral-400' : 'text-gray-600'}`}>Network Active</span>
      </div>

      {/* Transaction count */}
      <div className={`absolute top-4 right-4 text-xs ${isDark ? 'text-neutral-400' : 'text-gray-600'}`}>
